		...
		>
			...
			<el-form-item v-if="type!='info'"  label="预约内容" prop="yuyueneirong" >
				<editor 
					...
					class="editor"
					myQuillEditor="yuyueneirong"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.yuyueneirong" label="预约内容" prop="yuyueneirong" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.yuyueneirong"></span>
			</el-form-item>
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.yuyueneirong = this.ruleForm.yuyueneirong.replace(reg,'../../../springboot8w0i844u/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `yuyuezixunshi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.yuyuezixunshiCrossAddOrUpdateFlag = false;
											this.parent.search();
