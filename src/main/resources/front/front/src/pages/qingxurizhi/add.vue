<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="情绪分类" prop="qingxufenlei">
				<el-select v-model="ruleForm.qingxufenlei" placeholder="请选择情绪分类" :disabled=" false  ||ro.qingxufenlei" >
					<el-option
			...
			<el-form-item class="add-item" label="记录日期" prop="jiluriqi">
				<el-date-picker
					:disabled="true  ||ro.jiluriqi"
			...
			<el-form-item class="add-item" label="日志内容" prop="rizhineirong">
				<editor 
	...
	export default {
		...
		methods: {
			...
			init(type) {
				...
				this.$http.get('option/qingxufenlei/qingxufenlei', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						this.qingxufenleiOptions = res.data.data;
					}
				});
			...
			// 多级联动参数
			info() {
				this.$http.get(`qingxurizhi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`qingxurizhi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
