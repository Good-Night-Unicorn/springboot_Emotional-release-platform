<template>
	<div class="addEdit-block">
		...
		>
			...
			<el-form-item v-else-if="ruleForm.neirongmiaoshu" label="内容描述" prop="neirongmiaoshu" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.neirongmiaoshu"></span>
			</el-form-item>
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.neirongmiaoshu = this.ruleForm.neirongmiaoshu.replace(reg,'../../../springboot8w0i844u/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `qingxuxuanxie/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.qingxuxuanxieCrossAddOrUpdateFlag = false;
											this.parent.search();
