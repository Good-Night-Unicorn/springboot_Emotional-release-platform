		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">编曲</div>
						<div class="text "  >{{detail.bianqu}}</div>
					</div>
					<div class="item">
						<div class="lable">填词</div>
						<div class="text "  >{{detail.tianci}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">发行方</div>
						<div class="text "  >{{detail.faxingfang}}</div>
					</div>
					<div class="item">
						<div class="lable">版权方</div>
						<div class="text "  >{{detail.banquanfang}}</div>
					</div>
					<div class="item">
						<div class="lable">发行日期</div>
						<div class="text "  >{{detail.faxingriqi}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						...
						<audio v-if="detail.songfile" ref="songfile" :src="baseUrl + detail.songfile"  controls="controls" >
							您的浏览器不支持音频播放
						</audio>
					</div>
					<div class="btn_box">
	...
	export default {
		...
		methods: {
			...
			storeup(type) {
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'yinlexinxi', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			getThumbsupOrCrazilyStatus() {
				if(localStorage.getItem("frontToken")) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '21', refid: this.detail.id, tablename: 'yinlexinxi', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
							this.isThumbsupnum = true;
							this.thumbsupOrCrazilyInfo = res.data.data.list[0];
						}
					});
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '22', refid: this.detail.id, tablename: 'yinlexinxi', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
