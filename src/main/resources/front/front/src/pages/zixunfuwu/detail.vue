	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/zixunfuwu?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('zixunfuwu','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('zixunfuwu','删除')" @click="delClick">删除</el-button>
						<el-button class="chatBtn" v-if="btnAuth('zixunfuwu','私聊')&&detail.id!=mid" @click="chatClick">联系TA</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('zixunfuwu','在线预约')" 
							@click="onAcross('yuyuezixunshi','','','','')" type="warning">
							在线预约
						</el-button>
					</div>
				</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="服务内容" name="first">
					<div class="ql-snow ql-editor" v-html="detail.fuwuneirong"></div>
				</el-tab-pane>
			</el-tabs>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`zixunfuwu`);
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('zixunfuwu/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'zixunfuwu', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/zixunfuwu', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/zixunfuwuAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此咨询服务？') .then(_ => {
					this.$http.post('zixunfuwu/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
			...
			chatClick() {
				...
				if(localStorage.getItem('frontSessionTable')=='zixunfuwu') {
					if(this.detail.zixunshizhanghao == user.zixunshizhanghao){
						this.$message.error('不能给自己发信息')
						return false
					}
				}
				this.$http.get('zixunshi/query', {
					params: {
						zixunshizhanghao: this.detail.zixunshizhanghao
					}
				}).then(res => {
