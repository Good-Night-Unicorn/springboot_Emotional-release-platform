		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","flexWrap":"wrap","justifyContent":"space-between","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"margin":"10px 0","display":"block"}' >
					...
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}' class="select" label="情绪分类" prop="qingxufenlei">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">情绪分类</label>
						<el-select clearable v-model="searchForm.qingxufenlei" placeholder="请选择情绪分类" >
							<el-option v-for="(item,index) in qingxufenleiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
				...
				<el-row class="actions" :style='{"margin":"10px 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('qingxuwenzhang','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 0px","fontSize":"inherit","color":"inherit","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('qingxuwenzhang','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"0px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('qingxuwenzhang','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('qingxuwenzhang','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai11" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="btn8" v-if="isAuth('qingxuwenzhang','私聊')" type="success" @click="chatClick(scope.row)">
								<span class="icon iconfont icon-zhangjie8" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","height":"40px"}'></span>
								私聊
							</el-button>
							<el-button class="view" v-if="isAuth('qingxuwenzhang','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('qingxuwenzhang','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
<script>
	...
	export default {
		...
		methods: {
			...
			init () {
				this.$http({
					url: `option/qingxufenlei/qingxufenlei`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						this.qingxufenleiOptions = data.data;
					} else {
			...
			getDataList() {
				...
				if(this.searchForm.qingxufenlei!='' && this.searchForm.qingxufenlei!=undefined){
					params['qingxufenlei'] = this.searchForm.qingxufenlei
				}
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussqingxuwenzhang',query:{refid:id}});
			},
			// 删除
